function AddNewGeometry (size, X0, Y0, VX0, VY0, InitDist, Temp, Type)
global C
...
for row = 1:size
    space_between_y = (size - row) * AtomSpacing;
    for col = 1:row
        space_between_x = (col - 1) * AtomSpacing;

        x(end + 1) = X0 + space_between_x - (row -1) * AtomSpacing / 2;
        y(end + 1) = Y0 + space_between_y;
